...
class Stitch:
    ...
    def process(self,
        ...
        cv2.imwrite(save_dir + "/output082001.png", img_merge[0])
    ...
    def stitch(self, outWidth, imgt,
               ratio=0.75, reprojThresh=1.0):
        """两张图像拼接"""
        (kpsA, featsA) = self.extractFeatures(imgt[1])
        (kpsB, featsB) = self.extractFeatures(imgt[0])

        kpsMatches = self.matchKeypoints(kpsA,
                                         kpsB,
                                         featsA,
                                         featsB,
                                         ratio,
        ...
        res = cv2.warpPerspective(imgt[1], H, (imgt[1].shape[1], imgt[1].shape[0] + imgt[0].shape[0]))

        # add image to container
        res[0:imgt[0].shape[0], 0:imgt[0].shape[1]] = imgt[0]
        # container = self.addImage(res, container, transparent=False)  # 交集拼接
        # scaledContainer = self.scaleAndCrop(container, outWidth)
        scaledContainer = self.scaleAndCrop(res, outWidth)
...
if __name__ == "__main__":
    ...
    if 1:
        ...
        # imgs.loadFromDirectory("/home/pzw/hdd/dataset/stitch_data/01feichengdongpo3/")
